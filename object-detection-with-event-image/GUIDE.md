...
たとえば、出入り口や搬入口にカメラを設置して、イベント発生時の画像や動画で人の数や車の台数、何が記録されているかを確認を取りたい場合があるとします。
その際、手作業で確認する場合は、検知したイベントの数に比例して多くの時間を使ってしまいます。AI を利用して、画像や映像から物体が検出できれば自動で人数カウントや、何が記録されているかを確認できます。

ここでは、[SORACOM API](https://users.soracom.io/ja-jp/tools/api/) (以下、API) を使ったサンプルコードを実行することで、イベント画像に対して学習済みモデルを使った物体検出を体験できます。サンプルコードは、[Colaboratory](https://colab.research.google.com/)(以下、Colab) を使って実行します。
...
このページで使用するサンプルコードです。
  ...
  - [![Open In Colab Icon](https://colab.research.google.com/assets/colab-badge.svg "no-icon-external-link") api-examples-object-detection-with-event-image.ipynb](https://colab.research.google.com/github/soracom-labs/sora-cam-api-examples/blob/main/object-detection-with-event-image/api-examples-object-detection-with-event-image.ipynb)
...
1. サンプルコードの **[ステップ 1]** のコードセルにマウスポインターを合わせて **[▶]** クリックします。
    ...
    > [!NOTE]
    > **利用しているライブラリ**
    > - ここでは [ultralytics / ultralytics](https://github.com/ultralytics/ultralytics) を利用しています。
    > - 利用しているライブラリは [GNU Affero General Public License v3.0](https://github.com/ultralytics/ultralytics/blob/main/LICENSE) で提供されています。
...
> [!NOTE]
> 初回実行時に、物体検出に利用する学習済みモデル (例: `yolov8n.pt`) が自動的に Colab にダウンロードされます。
>
> ![](images/perform-object-detection-03.png)

1. サンプルコードの **[ステップ 10]** のコードセルにマウスポインターを合わせて **[▶]** クリックします。

    ![](images/perform-object-detection-01.png)

    実行結果に物体検出の実行状況が表示されます。

    ![](images/perform-object-detection-02.png)

    実行結果には、検出した物体の名称 (`# Object type name = `) と検出の適合率 (`# Precision of detection =`) が出力されます。

1. ![](images/icon-colab-folder.png) をクリックします。

    「runs/detect/predict」ディレクトリと、物体検出の結果画像ファイルが表示されます。

    ![](images/perform-object-detection-04.png)
...
1. サンプルコードの **[ステップ 11]** のコードセルにマウスポインターを合わせて **[▶]** クリックします。
    ...
    ![](images/display-detection-image-01.png)

    実行結果に、物体検出の結果画像が表示されます。

    ![](images/display-detection-image-02.png)
    ...
    > [!NOTE]
    > 物体検出の結果画像が正しく表示されない場合は ![](images/icon-colab-folder.png) をクリックして画像ファイルを直接確認してください。
...
1. ローカル環境にダウンロードされた zip 形式で圧縮したファイルを展開して、物体検出の結果画像ファイルが表示できることを確認します。
...
[SORACOM API](https://users.soracom.io/ja-jp/tools/api/) とライブラリを使って、ユーザーコンソールでは実現できない **イベント画像に映っている物体を検出する** サンプルコードを Colab で体験できました。ステップごとの解説とサンプルコードを参考に、実際のユースケースでも SORACOM API やライブラリを活用して、新しい使いかたや自動化、省力化にチャレンジしてください。
...
サンプルコードでは、物体検出の学習済みモデルを利用しました。今回利用したライブラリは他にも [学習済みモデル](https://github.com/ultralytics/ultralytics/tree/main/ultralytics/models) が用意されています。別の学習済みモデルを利用する場合には、サンプルコードの `detect_objects()` を呼び出す際、引数 `model_name` に任意のモデル名称を指定することで、指定した学習済みモデルで AI を試せます。その他詳しい内容はライブラリのページを参照してください。
